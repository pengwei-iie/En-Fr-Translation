...
# 每display_step轮打一次结果
display_step = 100
# max_source_sentence_length = 17 # data_process里面计算过
# max_target_sentence_length = 22
...
with train_graph.as_default():
    ...
    with tf.name_scope("optimization"):# 直接计算序列的损失函数, masks滤去padding的loss计算
        ...
        #     with tf.Session(graph=train_graph) as sess:
        #         sess.run(tf.global_variables_initializer())
        #         print(sess.run([training_logits[0][0][:], targets[0][:]], {inputs: source_batch, targets: target_batch, learning_rate: lr, source_sequence_len: sources_lengths, target_sequence_len: targets_lengths}))
    ...
    for epoch_i in range(epochs):
        for batch_i, (source_batch, target_batch, sources_lengths, targets_lengths, max_target_len) in enumerate(
            ...
            if batch_i % display_step == 0 and batch_i > 0:
                # inference_logits 这个地方应该inputs: source_batch,是与验证集的数据去预测，这里用的训练集的，然而好像也没有处理
                ...
                print('Blue is :', score / batch_size)
    # Save Model
